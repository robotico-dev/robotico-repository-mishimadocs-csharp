# Robotico.Repository.MishimaDocs
[![Woodpecker CI](https://dvalin.robotico.dev/api/badges/robotico/robotico-repository-mishimadocs-csharp/status.svg)](https://dvalin.robotico.dev/robotico/robotico-repository-mishimadocs-csharp)
...
[![NuGet](https://img.shields.io/badge/NuGet-Robotico.Repository.MishimaDocs-blue?logo=nuget)](https://github.com/robotico-dev/robotico-repository-mishimadocs-csharp/packages)
[![Robotico](https://img.shields.io/badge/Robotico-Tier%203%20adapter-1f883d?logo=github)](https://github.com/robotico-dev)

NuGet package id: **`Robotico.Repository.MishimaDocs`** — `IRepository<TEntity, TId>` and `IAsyncRepository<TEntity, TId>` over the **`MishimaDocs`** engine (JSON documents per entity id). This repo slug uses `mishimadocs` for checkout layout only.
...
`MishimaDocsUnitOfWork.CommitAsync` succeeds but does not batch repository operations; align expectations with `UnitOfWorkGuard` in apps that require deferred commits.
...
```bash
dotnet restore Robotico.Repository.MishimaDocs.sln
dotnet build Robotico.Repository.MishimaDocs.sln -c Release
dotnet test Robotico.Repository.MishimaDocs.sln -c Release
```

`MishimaDocsUnitOfWork` is a no-op `CommitAsync` (each repository call commits immediately through MishimaDocs). Use `IMishimaWriteBatch` in application code when you need atomic multi-document writes.

## CI and quality bar

GitHub Actions runs build, tests, coverlet, a line-coverage gate on **Robotico.Repository.MishimaDocs**, and — in the umbrella `csharp/` layout — `verify-robotico-library-bar.ps1`. Same rules as other Robotico C# libraries: one top-level type per file, explicit types (`.editorconfig` / `IDE0008`). See `csharp/build/ROBOTICO_LIBRARY_10_STANDARD.adoc`.
